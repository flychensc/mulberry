...
def gen_kline(data, fpath, mav):
    data = pd.DataFrame(data)
    data['datetime'] = data['datetime'].map(lambda x: dt.datetime.strptime(str(x), "%Y-%m-%d").date())
    data.index = pd.to_datetime(data.datetime)
    data = data.drop(columns=['datetime'])

    mpf.plot(data, type='candle', mav=mav, volume=True, style=my_style, axisoff=True, tight_layout=True, scale_padding=0, savefig={'fname': fpath})
...
def bluk():
    ...
    mav = (config.getint('MA', 'MA1'), config.getint('MA', 'MA2'), config.getint('MA', 'MA3'))
    ...
    for symbol in symbol_list:
        ...
        if len(stock_zh_a_hist_df) < PERIOD:
            ...
            gen_kline(stock_zh_a_hist_df[-PERIOD:], fpath, mav)
